...
class CNN(object):
    ...
    def cnn_op(self, x, dropout,sen_pre ,training, kernel_size = 3, stride_size = 1, activation=tf.nn.relu):
    ...
    def _acc(self,logits, outputs):
        ...
        return acc
    def classify(self,words,outputs,dropout,sen_pre, training):
        x = self.cnn_op(words,dropout,sen_pre, training)
...
class QASystem(object):    
    ...
    def get_feed_dict(self, questions, contexts, answers, cnn_output, dropout_val, training, lr=0.001):
        """
    ...
    def setup_system(self):
        ...
        cnn_logits, cnn_pred, cnn_acc, cnn_loss, cnn_repre, sen_repre = cnn.classify(encoded_passage,self.cnn_output,self.dropout,p_encode,self.training)

        
        q_ids = tf.reshape(self.question_ids,[-1])

        #duli
        self.cnn_repre = tf.get_variable(name='relation_embedding',
                                                    shape=[self.config.relation_num, self.config.embedding_dim],
                                                    dtype=tf.float32)
        cnn_repre=self.cnn_repre
    ...
    def setup_loss(self):
        ...
        # self.loss = tf.reduce_mean(match_loss)
        # self.loss = self.config.lambda_value*tf.reduce_mean(match_loss)+(1-self.config.lambda_value)*self.cnn_loss+0.001*tf.reduce_mean(reg)
    ...
    def answer_sequence(self, outputs):
        ...
        return res
    def evaluate_duli_cnn(self, session, dataset, config, mode):
        q, c, a, co = zip(*[[_q, _c, _a, _co] for (_q, _c, _a, _co) in dataset])
        input_feed =  self.get_feed_dict(q, c, a, co, 1.0, False)
        ...
        for i in range(sample):
            ...
            for k in range(1,len(cnn_pred[i])):
                ...
                if cnn_pred[i][k]==1 and gold_cnn[i][k]==1:
            ...
            pbar.set_description("pred: %s" % (str(pred)))
            pbar.update(1)
        f.close()
        pbar.close()

        print(' %f %f %f\n'%(pred,pred_cnn_cnt, true_cnn_cnt))
        if pred==0:
            p=0.0
            r=0.0
            f1=0.0
    ...
    def evaluate_cnn(self, session, dataset, config, mode):
        ...
        for i in range(sample):
            ...
            # for k in range(1,len(cnn_pred[i])):
            for k in range(1,len(gold_cnn[i])):
                ...
                if gold_cnn[i][k]==1:
                    ...
                    input_feed = self.get_feed_dict([[k]],[c[i]],[a[i]],[gold_cnn[i]],1.0, False)
                    ...
                    if gold_cnn[i][k]==1:
                        ...
                        for ind in range(len(new_pred_res)):
                            if new_pred_res[ind] in gold_match[k]:
                                em_score+=1.0
                    # temp['label']=[int(cnn_pred[i][k]),int(gold_cnn[i][k])]
                    temp['label']=[int(gold_cnn[i][k]),int(gold_cnn[i][k])]
    ...
    def run_epoch(self, session, train, lr):
        ...
        for i, (q_batch, c_batch, a_batch, co_batch) in enumerate(minibatches(train, self.config.batch_size)):
            ...
            input_feed = self.get_feed_dict(q_batch, c_batch, a_batch, co_batch, self.config.dropout_val, True, lr)
    ...
    def train(self, session, dataset, train_dir,config):
        ...
        train_flag=1
        best_em = 0.0
        # f1 = self.evaluate_duli_cnn(session, test, config, 'test')
        ...
        if train_flag:
            ...
            # dev_f1 = self.evaluate_duli_cnn(session, dev, config, 'dev')
            # f1 = self.evaluate_duli_cnn(session, test, config, 'test')
