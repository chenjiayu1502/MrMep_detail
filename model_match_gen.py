...
class Decoder(object):
    ...
    def run_multi_head_answer_ptr(self, encoded_rep, p_encode, masks, labels, config):
        with tf.variable_scope("decode_step") as scope:
            ...
            logits=[]
            if config.use_attention_type==0:
                att=self.attention
            else:
                att=self.self.my_attention_dot
            for step in range(config.max_decode_size):
...
class QASystem(object):    
    ...
    def setup_system(self):
        ...
        if self.config.use_match:
            ...
            # logits= decoder.decode_multi_head([encoded_question, encoded_passage], [p_encode, p_state], [self.question_lengths, self.passage_lengths], self.labels, self.config, self.dropout)
                    ...
                    if gold_cnn[i][k]==1:
                        ...
                        for ind in range(len(new_pred_res)):
                            if pred_res[ind] in gold_match[k]:
                                em_score+=1.0
                    temp['label']=[cnn_pred[i][k],gold_cnn[i][k]]
                    temp['pred']=pred_res
